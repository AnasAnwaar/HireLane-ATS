...
- [~] Notification templates — **email-only** (SMS/WhatsApp/Twilio out of scope by decision). Email layer done (`src/server/email/send.ts`, Resend); contact form + candidate portal links already send. Remaining: wire the candidate-lifecycle emails (assessment assigned, interview scheduled, offer/decision) through `sendEmail`
...
### 🚧 CP-28 — Super-Admin Portal (platform) — Phases 1–5 done; impersonation + API mgmt deferred
